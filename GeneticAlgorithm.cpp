...
public:
    ...
    void unit_sort(){//根据评估结果对个体进行排序
        for(int i = 0; i < unit_num; i++)
        {
            for(int j = i+1; j < unit_num; j++)
            {
                ...
                {
                    Individual temp;
                    memcpy(&temp, &group[i], sizeof(Individual));
                    memcpy(&group[i], &group[j], sizeof(Individual));
                    memcpy(&group[j], &temp, sizeof(Individual));
                }
            }
        }
    }
    
    void mutation(Individual &t){//突变
    ...
    Individual cross(Individual &father, Individual &mother){//交叉
        Individual son;
        int pos = 0;
        bool flag[city_num] = {};
        int l = rand()%city_num;
...
Individual group[unit_num];//种群变量
Individual bestone;//记录最短路径
